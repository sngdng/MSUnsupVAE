...
"""
semBLEU score implementation
adapted from https://github.com/freesunshine0316/sembleu/blob/master/src/bleu_score.py
"""
...
NgramInstance = namedtuple('NgramInstance', 'ngram length')
...
def corpus_bleu(list_of_references, hypotheses, weights=(0.34, 0.33, 0.33),
    ...
    for references, hypothesis in zip(list_of_references, hypotheses):
        ...
        # For each order of ngram, calculate ngramthe numerator and
        # denominator for the corpus-level modified precision.
        for i, _ in enumerate(weights, start=1):
            p_i = modified_precision_graph(references, hypothesis, i)
            if p_i == None:
                continue
        ...
        hyp_lengths += hyp_len
        ref_lengths += closest_ref_length_graph(references, hyp_len)
    ...
    p_n = [Fraction(p_numerators[i], p_denominators[i], _normalize=False) if p_numerators[i] > 0 and p_denominators[i] > 0 else 0
           for i, _ in enumerate(weights, start=1)]
...
def modified_precision_graph(references, hypothesis, n):
...
def closest_ref_length_graph(references, hyp_len):
    """
